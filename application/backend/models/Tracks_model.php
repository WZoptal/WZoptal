...
class Tracks_model extends CI_Model{
    function __construct(){
 	...
 	public function getpostsData($searcharray = array(), $user_type){
        ...
        $sql = "SELECT * FROM musics where status <> 4";
        if (isset($searcharray["search"]) && $searcharray["search"] != "")
        {
            $sql .= " AND trackname like '%" . $searcharray["search"] . "%' OR music_file like '%" . $searcharray["search"] . "%'";
        }
        if (isset($searcharray["page"]) && $searcharray["page"] != "")
        {
    ...
    public function view_track($id){
     ...
     public function get_gener_name($id){
        $this->db->select("*");
    ...
    public function enable_disable_track($id, $status){
        $this->db->where("id", $id);
        $arr = array("status" => $status);
        return $this->db->update("musics", $arr);
        //return $this->db->last_query();
    }
    public function archive_track($id){
        $this->db->where("id", $id);
        $arr = array("status" => 4);
        return $this->db->update("musics", $arr);
        //return $this->db->last_query();
    ...
    //for user category	
    public function getIndividualtrack($track_id){
        $this->db->select("*");
        $this->db->from('musics');
        $where = array("id" => $track_id, "status <> " => "4");
        $this->db->where($where);
        $query     = $this->db->get();
        //echo $this->db->last_query(); die;
        $resultset = $query->row_array();
