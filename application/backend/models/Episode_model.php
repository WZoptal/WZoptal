...
class Episode_model extends CI_Model {
	...
	public function getCategoryData($searchdata=array()){
		...
		if(isset($searchdata["search"]) && $searchdata["search"]!="" && $searchdata["search"]!="search"){
			$this->db->like("episodes.title",$searchdata["search"]);	
		}	
	...
	public function add_edit_episode($arr){//print_r($arr); die;
		if($arr["id"] == ""){
			...
			$arr["time"]   = time();
			$arr["status"] = 1;
			$res = $this->db->insert("episodes",$arr);	//echo $this->db->last_query(); die;
			return $res;
		}	
		else{
 			$arr["time"] = time();
			...
			$res = $this->db->update("episodes",$arr);  //echo $this->db->last_query(); die;
			if($res){
				$arr["updated_at"] = time();
				$arr1["time"] = time();
				$arr1["title"] = $arr["title"];
				$this->db->where("id", $arr["id"]);
				$this->db->update("episodes",$arr1);  //echo $this->db->last_query(); die;
			}
			return $res;
		}	
	}
	...
	public function deleteEpisode($id){
	  $this->db-> where('id',$id);
	 return $this-> db-> delete('episodes');
	}
	
	
	
	/*************************************Category function starts*****************************/
	public function getEpisodeData($searchdata=array()){
		$searcharray=array("status"=>"status");		
		if(!isset($searchdata["page"]) || $searchdata["page"]==""){
			$searchdata["page"]=0;	
		}
		...
		$where=array("episodes.status <>"=>"4", "episodes.status <>"=>"0");
		$this->db->where($where);		
		if(isset($searchdata["per_page"]) && $searchdata["per_page"]!=""){
			if(isset($recordperpage) && $recordperpage!="" && ($startlimit!="" || $startlimit==0)){
				$this->db->limit($recordperpage,$startlimit);
