...
class User_model extends CI_Model{
	...
	public function getNotificationListbyid($user_id, $limit, $no_of_post){
	...
	// public function getPosts($user_id){
		...
		// return $query->result_array();
	// }
	function getRaisedPostDataById($user_id){
		$this->db->select('*');
		$this->db->from('post_flags');
		$this->db->where(array('user_id' => $user_id, "status" => 1));
		$query = $this->db->get();
		 //echo $this->db->last_query(); die;
		$allpost = $query->result_array();
		$i=0;
	...
	//SELECT * FROM `post_flags` where user_id=76
	...
	public function location_list_by($lat,$long,$sort, $address){

		$sql="select * FROM (SELECT *, (3959 * acos (cos(radians('".$lat."'))* cos(radians( latitude))* cos(radians(longitude) - radians('".$long."') )+ sin(radians('".$lat."'))* sin(radians(latitude)))) AS distances FROM (stream)) AS s  WHERE status =1 AND distances < 30 AND address like '%mohali%'  order by $sort";
	...
	public function home_post_list($user_id,$lat,$long,$limit, $no_of_post,$userId,$search){ 
		$adduser = $search=='' ? "OR user_id IN (".$user_id.")" : "";

		 $sql  = "select * FROM (SELECT *, (3959 * acos (cos(radians('".$lat."'))* cos(radians( latitude))* cos(radians(longitude) - radians('".$long."') )+ sin(radians('".$lat."'))* sin(radians(latitude)))) AS distances FROM (stream)) AS s  WHERE status =1 AND distances < 30  $adduser and (address like '%".$search."%' OR restaurant_name like '%".$search."%' OR item_name like '%".$search."%' OR tags like '%".$search."%' OR comment like '%".$search."%') order by id desc"; 
		 $sql .= "  limit ".$limit.", ".$no_of_post; 
	...
	public function update_items($arr,$item_id){
  	...
  	public function item_list($user_id){
		$this->db->select('*');
		$this->db->from('items');
		$this->db->where(array("user_id"=>$user_id,"status"=>1));
		$query= $this->db->get();
	...
	public function checkUserBlocked($arr){ 
		...
		$where = "((user_id = '".$arr['user_id']."' AND other_id = '".$arr['other_id']."') OR (user_id = '".$arr['other_id']."' AND other_id = '".$arr['user_id']."'))";
		$this->db->where($where);
		$result = $this->db->get();
		 //echo $this->db->last_query(); die;
	...
	public function deleteUserBlocked($arr){
		$where = array("user_id" => $arr['user_id'], "other_id" => $arr['other_id']);
 		$this->db->where($where);
	...
	function getBlockedUserArr($user_id){
		$this->db->select('*');
		$this->db->from('block_user');
		$this->db->where(array('user_id' => $user_id));
		$query = $this->db->get();
		 //echo $this->db->last_query(); die;
		$allblocked = $query->result_array();
		$i=0;
		foreach ($allblocked as $key => $value) {
			$blocked[] = $value['other_id'];
			$i++;
		}

		return $blocked;
	}
	function checkblockUser($user_id,$other_id){
		$sql  = "SELECT * FROM `block_user` WHERE (user_id=".$user_id." AND other_id=".$other_id.") OR (user_id=".$other_id." AND other_id=".$user_id.") ";
		$result = $this->db->query($sql);
		return $result->num_rows();
	}
	
	function getBlockedUserArr1($user_id){ 
		$this->db->select('*');
		$this->db->from('block_user');
		$where = "(user_id = '".$user_id."' OR other_id = '".$user_id."') ";
		$this->db->where($where);
		$query = $this->db->get();
		 //echo $this->db->last_query(); die;
		$allblocked = $query->result_array(); 
		...
		foreach ($allblocked as $key => $value) {
			if($user_id!=$value['user_id']){
				$blocked[] = $value['other_id'];
			}else if($user_id!=$value['other_id']){
				$blocked[] = $value['user_id'];
	...
	public function raiseFlagPostDB($arr){
  		$arr["time"]   = time();
  		$arr["posted"] = time();
 		return $this->db->insert("post_flags", $arr);
	}

	public function approved_Post_raised($user_id,$post_id){ 
			$arr['status'] = 1; 
			$this->db->where(array("user_id"=>$user_id, "post_id"=>$post_id, "status"=>0));
			//echo $this->db->last_query();die;
			return $this->db->update('post_flags', $arr);

	}
	public function approved_Post_raise_up($user_id,$post_id){ 
			$arr['status'] = 0; 
			$this->db->where(array("user_id"=>$user_id, "post_id"=>$post_id, "status"=>0));
			//echo $this->db->last_query();die;
			return $this->db->update('post_flags', $arr);
	...
	public function flag_exist($user_id,$post_id){
		$sql  = "SELECT * FROM `post_flags` WHERE user_id=".$user_id." AND post_id=".$post_id." GROUP BY post_id";
		$result = $this->db->query($sql);
		return $result->num_rows();
	}
	...
	public function near_me_places($user_id,$search,$lat,$long,$limit, $no_of_post, $distance, $sort_by_tag){
		$search =$sort_by_tag!='' ? $sort_by_tag : $search;
		if(!empty($search)){
			$where=" AND id !=".$user_id." AND user_type =2 AND status <> 4 AND archive = 0 AND (location LIKE '%".$search."%' OR username LIKE '%".$search."%'  OR tags LIKE '%".$search."%')  order by id DESC";
		}else{
    ...
    public function near_me_places2($user_id,$search,$lat,$long,$limit, $no_of_post, $distance, $sort_by_tag){
		$search =$sort_by_tag!='' ? $sort_by_tag : $search;
		if(!empty($search)){
			$where=" AND id !=".$user_id." AND user_type =2 AND status <> 4 AND archive = 0 AND (location LIKE '%".$search."%' OR username LIKE '%".$search."%'  OR tags LIKE '%".$search."%')  order by id DESC";
		}else{
		...
		$sql  = "select id FROM (SELECT *, (3959 * acos (cos(radians('".$lat."'))* cos(radians( latitude))* cos(radians(longitude) - radians('".$long."') )+ sin(radians('".$lat."'))* sin(radians(latitude)))) AS distances FROM (users)) AS i  WHERE id <> '".$user_id."' AND distances < ".$distance." ".$where;
		$this->db->limit($no_of_post,$limit);
		$result = $this->db->query($sql);
		//echo $this->db->last_query(); die;
		return $result->result_array();
...
public function get_suggestion_data($user_id){
	...
	function getDataItemsByID_all($item_id){
		$this->db->select('*');
		$this->db->from('items');
		$this->db->where(array('user_id' => $item_id,"status" => 1));
		$query = $this->db->get();
		return $query->result();
		
	}

	function checkfollowStatus($userid,$other_id){
		
		$this->db->select('*');
		$this->db->from('follower');
		$this->db->where(array('user_id' => $userid,'other_id' => $other_id));
		$query = $this->db->get();
		//echo $this->db->last_query(); die;
		if($query->num_rows() > 0){
			return $query->row_array();
	...
	function calculateRating($business_id){
		
		$this->db->select('avg(average_rating) as average_rating');
		$this->db->from('stream');
		$this->db->where(array('business_id' => $business_id ,'average_rating<>'=>0));
		$query = $this->db->get();
		//echo $this->db->last_query(); die;
		$data =  $query->row_array();
		return $data['average_rating']!='' ? number_format(max(0, $data['average_rating']),1) : 0;
	}

	function getDataItems_all($limit, $no_of_post){
		$this->db->select('*');
		$this->db->from('items');
	...
	function itemRating($item_id){
		
		$this->db->select('avg(average_rating) as average_rating');
		$this->db->from('stream');
		$this->db->where(array('item_id' => $item_id ,'average_rating<>'=>0));
		$query = $this->db->get();
		//echo $this->db->last_query(); die;
		$data =  $query->row_array();
		return $data['average_rating']!='' ? number_format(max(0, $data['average_rating']),1) : 0;
	}
	public function delete_items($arr){
		$where="  id =".$arr['item_id']." AND status =1";
	   $this->db-> where($where);
	  //echo $this->db->last_query(); die;
